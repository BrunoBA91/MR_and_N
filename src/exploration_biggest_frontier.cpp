...
geometry_msgs::Pose decideGoal()
{
  ...
  int N_front = frontiers_.size();
	...
	//look for the biggest reacheable boundary
	for (int i=0;i<N_front;i++)
	{
		if (frontiers_[i].size >= maxsize)
		{
			ROS_INFO("Biggest boundary: %d",frontiers_[i].id);
			big_front = i;
      maxsize = frontiers_[i].size;
			if (isValidPoint(frontiers_[i].free_center_point))
      {
				...
				ROS_INFO("Biggest boundary reacheable");
			}
		}
	}
